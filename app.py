...
.expando {background:linear-gradient(145deg,#f8fafc,#e2e8f0);border-radius:10px;padding:1rem;margin:1rem 0;}
...
def get_nifty_data(symbol):
    try:
        ...
        if len(hist) >= 30:
            ...
            macd_line, signal_line = macd.macd().iloc[-1], macd.macd_signal().iloc[-1]
            ...
            # SIGNAL LOGIC
            rsi_super, rsi_buy, rsi_sell = rsi < 35, rsi < 45, rsi > 65
            macd_bull, ma_bull = macd_line > signal_line, price > ma20
            confirmations = sum([rsi_super, macd_bull, ma_bull])
            
            if confirmations == 3: signal, signal_class = '🚀 SUPER BUY', 'super-buy'
            elif confirmations >= 2: signal, signal_class = '🟢 STRONG BUY', 'strong-buy'
            elif rsi_buy or macd_bull: signal, signal_class = '🟢 BUY', 'buy'
            elif rsi_sell or not macd_bull: signal, signal_class = '🔴 SELL', 'sell'
            else: signal, signal_class = '🟡 HOLD', 'hold'
            
            return {
                'Stock': symbol, 'Price': f"₹{price:.0f}", 'Change': f"{change:.1f}%",
                'RSI': f"{rsi:.1f}", 'MACD': f"{macd_line:.2f}", 'Signal_Line': f"{signal_line:.2f}",
                'MA20': f"₹{ma20:.0f}", 'Price/MA20': '📈' if ma_bull else 
